...
// Reused across calls to avoid a fresh O(k) allocation on every single
// frame — high-degree draws happen often enough (soliton spike) that this
// was measurable GC pressure at sustained tx fps.
let fyScratch = new Uint32Array(0);
...
function frameIndices(k: number, cdf: Float64Array, sessionId: number, seq: number): number[] {
  ...
  if (d > k >> 3) {
    ...
    if (fyScratch.length !== k) fyScratch = new Uint32Array(k);
    for (let i = 0; i < k; i++) fyScratch[i] = i;
    const out: number[] = new Array<number>(d);
    for (let i = 0; i < d; i++) {
      const j = i + (rnd() % (k - i));
      const t = fyScratch[i]!;
      fyScratch[i] = fyScratch[j]!;
      fyScratch[j] = t;
      out[i] = fyScratch[i]!;
    }
    return out;
