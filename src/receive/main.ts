...
import { fnv1a, parseFrame, parseMetaPayload } from "../core/protocol";
...
function finish(payload: Uint8Array, hashOk: boolean, seconds: number, totalLen: number) {
  ...
  const fileBytes = meta?.data ?? payload;
  const fileName = meta?.name || "received.bin";
  ...
  const blobUrl = URL.createObjectURL(new Blob([fileBytes as BlobPart], { type: mime }));
